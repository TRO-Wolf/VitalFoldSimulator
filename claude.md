...
→ 202 Accepted { "job_id": "<uuid>", "status": "running" }
...
- All inserts use **batch inserts** via SQLx where possible (`INSERT INTO ... VALUES (...), (...)`).
...
- Aurora DSQL IAM tokens expire in ~15 minutes. The initial build generates one token at startup; add a background renewal task for production.
...
- Rate limiting
- Automatic IAM token renewal background task
- Integration tests
