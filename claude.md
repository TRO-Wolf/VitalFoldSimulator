...
`provider_id` is `BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)` — Rust type `i64`.
`license_type` is one of `"MD"`, `"DO"`, `"NP"` (~30% NPs).
Email format: `{first_initial}.{lastname}@example.org`.
...
`clinic_id` is `BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)` — Rust type `i64`.
Email format: `vfhc_{city}{n}@vitalfold.org`. Street address: `"1234 Elm Blvd, Suite 200"`.
...
Time range: 8:00 AM – 4:45 PM (36 slots per provider per day).
...
pub struct SimulationContext {
    ...
    pub clinic_ids: Vec<i64>,                    // BIGINT identity
    pub provider_ids: Vec<i64>,                  // BIGINT identity
    ...
    // Patient-to-clinic index mapping (for geographic bias in appointment gen)
    pub patient_home_clinics: Vec<usize>,
    // Provider-to-clinic index mapping (for proportional distribution)
    pub provider_clinic_assignments: Vec<usize>,
}

/// Defaults: plans_per_company=3, providers=50, patients=50000, records_per_appointment=1
/// Appointment volume is provider-driven: each provider fills 36 slots/day (SLOTS_PER_PROVIDER).
pub struct SimulationConfig {
    ...
    pub clinic_weights: Vec<u32>,        // default: DEFAULT_CLINIC_WEIGHTS
}
...
- Each provider fills 36 time slots per day (8:00–16:45 in 15-minute windows) at their assigned clinic
- Patients are randomly assigned to appointments, biased 70% toward the clinic's metro area
...
- Appointment times are generated in **15-minute windows**: `:00`, `:15`, `:30`, `:45` only.
- Hours range from 9 AM to 4:45 PM (hour 9..16, minute 0/15/30/45).
- This gives 32 possible time slots per day (8 hours × 4 windows).
- The column is `appointment_datetime` (TIMESTAMP, no timezone).
