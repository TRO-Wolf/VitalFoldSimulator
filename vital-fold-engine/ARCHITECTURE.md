...
Generated by `vital-fold-engine/migrations/init.sql` at setup (all 16 tables):
...
vital_fold.survey                              -- optional 1:1 with patient_visit (~30%)
  ├─ survey_id (UUID)
  ├─ patient_visit_id (UUID FK → patient_visit)
  ├─ gene_prissy_score (INT)                   -- 1-10, gold-layer provider quality metric
  ├─ experience_score (INT)                    -- 1-10
  ├─ feedback_comments (TEXT NULL)             -- 40% of surveys include free-text
  └─ creation_time (TIMESTAMP)

vital_fold.cpt_code                            -- reference table, seeded by init-db
  ├─ cpt_code_id (BIGINT identity)
  ├─ code (VARCHAR 5)                          -- '99213', '93000', etc.
  ├─ short_description (VARCHAR)
  ├─ category (VARCHAR)                        -- 'E/M' or 'Diagnostic'
  ├─ work_rvu, pe_rvu_nonfacility, pe_rvu_facility, mp_rvu (DECIMAL)
  ├─ global_days (SMALLINT NULL)
  ├─ effective_year (SMALLINT)
  └─ is_active (BOOLEAN)
  UNIQUE (code, effective_year)

vital_fold.appointment_cpt                     -- 1+ line-items per appointment
  ├─ appointment_cpt_id (UUID)
  ├─ appointment_id (UUID FK → appointment)
  ├─ cpt_code_id (BIGINT FK → cpt_code)
  ├─ provider_id, clinic_id (BIGINT FKs)
  ├─ service_date (DATE)
  ├─ units (SMALLINT)
  ├─ modifier_1, modifier_2 (VARCHAR 2 NULL)
  ├─ work_rvu_snapshot, pe_rvu_snapshot, mp_rvu_snapshot (DECIMAL) -- RVU at service time
  ├─ total_rvu_snapshot (DECIMAL)
  ├─ conversion_factor (DECIMAL)               -- $32.7442 for CY2024
  ├─ expected_amount (DECIMAL NULL)            -- total_rvu × CF, rounded to 2dp
  └─ creation_time (TIMESTAMP)
```
...
Seeds date-dependent data for a configurable date range (7 steps):
...
6. Surveys — ~30% of visits generate a survey row with `gene_prissy_score` and `experience_score` (1–10) for gold-layer provider quality metrics
7. Billing line-items — one E/M CPT per appointment (cardiology-weighted: 99213 40%, 99214 35%), plus CPT 93000 when `ekg_usage = true`. Snapshots wRVU / PE RVU / MP RVU values for gold-layer productivity rollups.
